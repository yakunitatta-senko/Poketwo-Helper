...
class ImageGenerator:
    def __init__(self, ctx):
        ...
        self.bot_name = ctx.me.display_name
        self.text2_options = [
        ...
        self.config_path = "data/commands/help/option_image.json"
        self._load_config()
    ...
    def _load_config(self):
        ...
        self.color_replacements_map = config["color_replacements_map"]
        self.character_scale = config["character_scale"]
        self.character_aspect_ratio = config.get("character_aspect_ratio", None)
        self.scale_factor = config.get("scale_factor", 1.5)

        self.header_font_color = config["header_font_color"]
        self.base_font_color = config["base_font_color"]
        self.command_font_color = config.get("command_font_color", "white")

        self.base_font_size = int(config["base_font_size"] * self.scale_factor)
        self.command_font_size = int(config.get("command_font_size", 13) * self.scale_factor)
        self.header_font_size = int(config.get("header_font_size_factor", 35) * self.scale_factor)

        self.character_pos = tuple(int(v * self.scale_factor) for v in config["character_pos"])
        self.text_x_offset = int(config["text_x_offset"] * self.scale_factor)
        self.text_y_offset = int(config["text_y_offset"] * self.scale_factor)
        self.text_spacing = int(config["text_spacing"] * self.scale_factor)

    def reload_config(self):
        """Reload the configuration from the JSON file and update resources."""
        self._load_config()
        self._load_resources()
    ...
    def _download_image(self, url):
        # Placeholder for downloading image; implement as needed
        raise NotImplementedError("Image download functionality not implemented")

    def _resize_character(self):
        """Resize the character image proportionally and crop to target aspect ratio if specified."""
        # First, scale preserving original aspect ratio
        new_width = round(self.character.width * self.character_scale)
        new_height = round(self.character.height * self.character_scale)
        scaled = self.character.resize((new_width, new_height), Image.LANCZOS)

        # If aspect ratio is specified, crop to it
        if self.character_aspect_ratio:
            ratio_parts = self.character_aspect_ratio.split(':')
            target_w_ratio = float(ratio_parts[0])
            target_h_ratio = float(ratio_parts[1])
            target_aspect = target_w_ratio / target_h_ratio
            scaled_aspect = scaled.width / scaled.height

            if scaled_aspect > target_aspect:
                # Image is wider than target; crop width
                crop_width = int(scaled.height * target_aspect)
                left = (scaled.width - crop_width) // 2
                right = left + crop_width
                top = 0
                bottom = scaled.height
            else:
                # Image is taller than target; crop height
                crop_height = int(scaled.width / target_aspect)
                top = (scaled.height - crop_height) // 2
    ...
    def create_image(self):
        ...
        if os.path.exists(self.config_path) and os.path.getmtime(self.config_path) > self.last_config_mtime:
            self.reload_config()
            self.last_config_mtime = os.path.getmtime(self.config_path)

        # Update dynamic texts
        self.text1 = self._truncate_text(f"{self.bot_name} Help", 350 * self.scale_factor)
...
class Help_Thumbnails:
    def __init__(self, json_file_path):
        ...
        if os.path.exists(self.json_file_path):
            self.last_mtime = os.path.getmtime(self.json_file_path)
    ...
    def get_image_url(self, cog_name):
        # Check for updates
        if os.path.exists(self.json_file_path) and os.path.getmtime(self.json_file_path) > self.last_mtime:
            self.help_embed = self.load_images()
            self.last_mtime = os.path.getmtime(self.json_file_path)
