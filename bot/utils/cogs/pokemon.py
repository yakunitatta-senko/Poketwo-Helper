import os, re, csv, json, asyncio, traceback, io, base64, logging
...
class Pokemon_Emojis(commands.Cog):
    def __init__(self, bot):
        ...
        self.emoji_mapping = self.load_emoji_mapping()
        self.api_url = f"https://discord.com/api/v10/applications/{bot.user.id}/emojis"
        self.headers = {"Authorization": f"Bot {bot.http.token}"}
    ...
    def get_emoji_limit(self):
        return 256 * 1024

    def resize_image_for_discord(self, image_data, guild=None):
        try:
            max_size_bytes = self.get_emoji_limit()
    ...
    def validate_image_size(self, image_data, guild=None):
        ...
        max_size = self.get_emoji_limit()
        size = len(image_data)
        if size > max_size:
            return False, f"Image too too large: {size} bytes (limit {max_size})"
        return True, "OK"
    ...
    async def list_bot_emojis(self):
        try:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(self.api_url, headers=self.headers) as resp:
                    if resp.status == 200:
                        data = await resp.json()
                        emojis = list(data.values()) if isinstance(data, dict) else data
                        return {e["name"]: e["id"] for e in emojis if "name" in e and "id" in e}
                    else:
                        self.logger.error(f"Failed to fetch bot emojis: {resp.status}")
                        return {}
        except Exception as e:
            self.logger.error(f"Error listing bot emojis: {e}")
            return {}

    async def download_pokemon_images(self):
        self.logger.info("Starting Pokémon image download...")
        pokemon_ids = await self.fetch_all_pokemon_ids()
        existing = set(self.load_images())
        missing = [pid for pid in pokemon_ids if f"{str(pid).zfill(3)}.png" not in existing and pid not in self.failed_downloads]
        if not missing:
            self.logger.info("No missing Pokémon images.")
            return
        connector = aiohttp.TCPConnector(limit=100, limit_per_host=20)
        timeout = aiohttp.ClientTimeout(total=30, connect=10)
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            sem = asyncio.Semaphore(50)
            tasks = [self._download_with_semaphore(sem, session, pid) for pid in missing]
            for _ in tqdm(asyncio.as_completed(tasks), total=len(tasks), desc="Downloading Pokémon", ncols=80):
                await _
        self.logger.info(f"Downloaded {len(missing) - len(self.failed_downloads)}/{len(missing)} successfully")

    async def _download_with_semaphore(self, semaphore, session, pid):
        async with semaphore:
            success = await self.download_single_image(session, pid)
            if not success:
                self.failed_downloads.add(pid)
            return success

    async def download_single_image(self, session, pid):
        img_path = os.path.join(self.POKEMON_IMAGES_FOLDER, f"{str(pid).zfill(3)}.png")
        for url_template in self.IMAGE_SOURCES:
            try:
                async with session.get(url_template.format(pid)) as resp:
                    if resp.status == 200:
                        content = await resp.read()
                        resized = await asyncio.get_event_loop().run_in_executor(None, self.resize_image_for_discord, content)
                        if resized:
                            ok, msg = self.validate_image_size(resized)
                            if ok:
                                await asyncio.get_event_loop().run_in_executor(None, self._write_image_file, img_path, resized)
                                return True
                            else:
                                self.logger.error(f"Invalid image size for {pid}: {msg}")
            except Exception as e:
                self.logger.debug(f"Error downloading {pid}: {e}")
        return False
    ...
    async def fetch_all_pokemon_ids(self):
        self.logger.info("Fetching all Pokémon IDs...")
        ids = []
        invalid_ids = {10265, 10266, 10267, 10268, 10269, 10264, 10270, 10271}
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as s:
            ...
            while url:
                ...
                async with s.get(url) as r:
                    ...
                    d = await r.json()
                    all_pokemon.extend(d["results"])
                    url = d.get("next")
            self.logger.info(f"Found {len(all_pokemon)} Pokémon across {pages} pages.")
            sem = asyncio.Semaphore(30)
            tasks = [self._fetch_pokemon_data(sem, s, p, invalid_ids) for p in all_pokemon]
            results = await asyncio.gather(*tasks)
            ids = [r for r in results if isinstance(r, int)]
        return sorted(ids)

    async def _fetch_pokemon_data(self, sem, s, result, invalid_ids):
        async with sem:
            try:
                async with s.get(result["url"]) as r:
                    if r.status == 200:
                        d = await r.json()
                        i = d["id"]
                        if i not in invalid_ids: return i
            except Exception as e:
                self.logger.debug(f"Error fetching data for {result.get('name')}: {e}")
        return None

    async def upload_single_emoji(self, pid):
        existing = await self.list_bot_emojis()
        name = str(pid).zfill(3)
        if name in existing:
            if str(pid) not in self.emoji_mapping:
                self.emoji_mapping[str(pid)] = {"name": name, "id": existing[name]}
                await asyncio.get_event_loop().run_in_executor(None, self._save_emoji_mapping)
            return False
        if str(pid) in self.emoji_mapping:
            return False
        if pid in self.failed_downloads:
            return False
        path = os.path.join(self.POKEMON_IMAGES_FOLDER, f"{name}.png")
        data = await asyncio.get_event_loop().run_in_executor(None, self._read_image_file, path)
        if not data:
            self.logger.error(f"Missing image for {pid}")
            return False
        ok, msg = self.validate_image_size(data)
        if not ok:
            resized = await asyncio.get_event_loop().run_in_executor(None, self.resize_image_for_discord, data)
            if not resized: return False
            ok, msg = self.validate_image_size(resized)
        ...
        async with aiohttp.ClientSession(timeout=timeout) as s:
            async with s.post(self.api_url, headers=self.headers, json=payload) as r:
                if r.status == 201:
                    res = await r.json()
                    self.emoji_mapping[str(pid)] = {"name": name, "id": res["id"]}
                    await asyncio.get_event_loop().run_in_executor(None, self._save_emoji_mapping)
                    self.logger.info(f"✅ Uploaded Pokémon {pid}")
                    return True
                if r.status == 429:
                    wait = int(r.headers.get("Retry-After", 30))
                    self.logger.warning(f"Rate limited ({wait}s)")
                    await asyncio.sleep(wait)
                    return await self.upload_single_emoji(pid)
                self.logger.error(f"Failed to upload {pid}: {r.status}")
        return False
    ...
    async def upload_emojis_to_bot(self, global_existing, embed_message=None, ctx=None, embed=None):
        self.logger.info("Uploading emojis to bot...")
        bot_emojis = await self.list_bot_emojis()
        cap = 250
        if len(bot_emojis) >= cap:
            msg = "ERROR: Bot at emoji capacity!"
            self.logger.warning(msg)
            if embed and embed_message:
                embed.description = msg
                await embed_message.edit(embed=embed)
            elif ctx:
                await ctx.send(msg)
            return
        available = cap - len(bot_emojis)
        self.logger.info(f"Bot has {available} slots available")
        imgs = self.load_images()
        pids = []
        for img in imgs:
            try:
                pid = int(self.get_pokemon_id(img))
                if str(pid) not in global_existing and pid not in self.failed_downloads:
                    pids.append(pid)
            except: continue
        if not pids:
            self.logger.info("No new Pokémon to upload.")
            return
        if len(pids) > available:
            self.logger.warning(f"Only {available} slots available, but {len(pids)} to upload. Proceeding with all.")
        sem = asyncio.Semaphore(5)
        uploaded = 0
        async def upload_task(pid):
            nonlocal uploaded
            async with sem:
                if await self.upload_single_emoji(pid):
                    uploaded += 1
                    await asyncio.sleep(0.5)
        with tqdm(total=len(pids), desc="Uploading Emojis", ncols=80) as pbar:
            for _ in asyncio.as_completed([upload_task(p) for p in pids]):
                await _
                pbar.update(1)
        msg = f"✅ Upload complete! Added {uploaded}/{len(pids)} emojis."
    ...
    async def create_emojis(self, ctx):
        ...
        self.emoji_mapping = self.load_emoji_mapping()
        global_existing = set(self.emoji_mapping.keys())
        self.logger.info(f"Found {len(global_existing)} existing emojis")
        bot_emojis = await self.list_bot_emojis()
        if len(bot_emojis) >= 250:
            embed.description = "ERROR: Bot at emoji capacity!"
        ...
        embed.description = "Uploading emojis to bot..."
        embed.color = discord.Color.yellow()
        await initial_message.edit(embed=embed)
        await self.upload_emojis_to_bot(global_existing, embed_message=initial_message, ctx=ctx, embed=embed)

    def get_emoji_for_pokemon(self, pokemon_id):
        if str(pokemon_id) in self.emoji_mapping:
            emoji_data = self.emoji_mapping[str(pokemon_id)]
            return f"<:{emoji_data['name']}:{int(emoji_data['id'])}>"
        return None

    def call_emoji(self, emoji_mapping, pokemon_id):
        pokemon_id = pokemon_id['id']
        if str(pokemon_id) in emoji_mapping:
            emoji_data = emoji_mapping[str(pokemon_id)]
            return f"<:{emoji_data['name']}:{int(emoji_data['id'])}>"
        return None
    ...
    async def force_download(self, ctx, start_id: int = 1, end_id: int = 2000):
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            ...
            with tqdm(total=total_batches, desc="Force download batches") as pbar:
